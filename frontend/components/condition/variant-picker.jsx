...
export function VariantPicker({ value = [], onChange = () => {} }) {
	...
	async function fetchData() {
		...
		const response = await AgHelpers.request("/shopify/variant", { condition });
		...
		if (products) {
			const newValue = products
				.map(function (product) {
					return product.variants.map((variant) => variant.id.replace(VariantIdPrefix, ""));
				})
				.flat();
			const newVariants = products
				.map(function (product) {
