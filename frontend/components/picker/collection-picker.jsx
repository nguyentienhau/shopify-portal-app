...
function getSelectionIds(selected) {
	return selected.map((item) => CollectionIdPrefix + item.id);
}

function getSelectedData(selected) {
	return selected.map((item) => item.id.replace(CollectionIdPrefix, ""));
}

export function CollectionPicker({ data = [], resource = [], onChange = () => {} }) {
	...
	async function handleOpenModal() {
		...
		if (collections) {
			const newData = getSelectedData(collections);
			onChange(newData);
		}
	}
